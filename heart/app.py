...
def predict():
    ...
    if  request.method == "POST":
        ...
        if(MLoption == 'svm'):
            ...
            #m="SVM model with accuracy 86%"
        elif MLoption== 'nb':
            prediction=models.nb(data)
            #m="Bernoulli Naive Bayies model with accuracy 82%"
        elif MLoption =='rf':
            prediction=models.randomF(data)
            #m="random Forest model with accuracy 82%"
        elif MLoption == 'lr':
            prediction=models.logisticReg(data)
            #m="logistic regression model with accuracy 87%"
